...
if (isInitialized) {
  ...
  taskForm.addEventListener("submit", async (e) => {
    ...
    const newTask = {
      ...
      duration: duration,
      createdAt: Date.now(),
      reminders: deadline ? getReminderTimes(deadline) : {}
    };
...
// smart reminder
function getReminderTimes(deadline) {
  ...
  // 1 hour before
  const oneHourBefore = new Date(d);
  oneHourBefore.setHours(d.getHours() - 1);
...
console.log(window.reminderInterval);
// check reminders
function checkReminders() {
  const now = Date.now();
  triggerNotification({ title: "Test Task" }, "test");
  console.log("Triggered!");
  tasks.forEach(task => {
    if (!task.reminders) return;

    Object.entries(task.reminders).forEach(([key, time]) => {
      const reminderTime = Number(time);

      if (Math.abs(now - reminderTime) < 60000) {
        triggerNotification(task, key);
      }
    });
  });
}

function triggerNotification(task, type) {
  let message = "";

  if (type === "dayBefore") {
    message = `Reminder: "${task.title}" is due tomorrow`;
  } else if (type === "sameDay") {
    message = `Today deadline: "${task.title}"`;
  } else if (type === "oneHourBefore") {
...
import { requestForToken } from "./firebase-config.js";
console.log(navigator.serviceWorker.controller);
console.log('PushManager' in window);
console.log('serviceWorker' in navigator);
console.log(await import("https://www.gstatic.com/firebasejs/10.12.2/firebase-messaging.js"));
...
async function initNotifications() {
  try {
    ...
    // 🔥 CRITICAL FIX: ensure SW controls page
    await navigator.serviceWorker.ready;

    if (!navigator.serviceWorker.controller) {
      console.log("No SW controller yet — reloading page");
      location.reload();
      return;
    }

    console.log("SW is controlling page");

    // small delay ensures push system is attached
...
initNotifications();
console.log(typeof checkReminders);
